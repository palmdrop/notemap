...
**This is not second-brain software** — those exist in abundance, and the vault already is one. This is a *note-taking and note-processing flow*: capture and triage. The app is a conveyor belt with an inbox at one end and other people's archives at the other; items are supposed to *leave* it. (Recorded for later, not a change to this core: [project collections](project-collections.md) — collection/research/moodboard gathering as one more thing captures can flow into, whether that home ends up in-app, in the vault, or elsewhere. Kept open for now.)
