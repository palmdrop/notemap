...
**This is not second-brain software** — those exist in abundance, and the vault already is one. This is a *note-taking and note-processing flow*: capture and triage. The app is a conveyor belt with an inbox at one end and other people's archives at the other; items are supposed to *leave* it. ([Project collections](project-collections.md) — collection/research/moodboard gathering for creative projects — is one more thing captures flow into; it is a **destination**, reached through an adapter, never something the hub stores itself. See [pool-and-routing.md](pool-and-routing.md#collections-are-destinations).)
...
The concrete model behind this loop — the immutable feed under the queue, linear processing, the two-phase classify-then-route split, and the destination adapters — is settled in [pool-and-routing.md](pool-and-routing.md). One clarification it makes to the sentence above: the **queue** drains, but the **feed** it is a view over never loses anything, which is what lets long-lived creative material coexist with inbox-zero discipline.
...
~~Deferred, deliberately: **how "processed" is marked** in this view.~~ **Resolved** in [pool-and-routing.md](pool-and-routing.md#how-processed-is-marked): routing marks an item processed, archiving marks it processed-without-routing, and scrolling past marks nothing — that's a skip. No extra mechanism needed, and the continuous document *is* the linear queue rather than a second surface.
