...
- [x] Branch — landed as `agent/client-minted-assets`, the health half having taken
      `agent/pool-identity-and-health` ([rollout](offline-capture-rollout.md))
- [x] ADR 0022: the uploader mints the asset id. Record the rejected alternative — deferring the
...
- [x] `assets.store` takes the id from the caller instead of `ports.ids.next`. Its result gains a
      shape like `CaptureOutcome`'s: stored, or already-stored, so the route can answer `201` or
      `200` without a second read
- [x] Replay and conflict: an id already held answers that asset when the blob hash, filename and
...
- [x] `asset-id-conflict` is its own `AssetStoreRefusal` in `types/api/refusal.ts` rather than a
      third member of `AssetRefusal`, which is the read union `assets.open` answers with: widening
      it would have advertised a variant a read can never return, and every other operation in that
      file already has a union of its own
- [x] Tests beside `pool/assets.ts`: a first upload; the same bytes, name and type replayed; each of
...
- [x] `PUT /v1/assets/{id}` replacing `POST /v1/assets`: the same raw body, the same required
...
- [x] **The media-type guard's carve-out becomes a pattern.** http-v1.md says the raw-body exception
...
- [x] `asset-id-conflict` joins the refusal table, the status mapping in `errors/refusals.ts`, and
...
- [x] `tests/seed` uploads under ids it mints, and its pre-check for an item already captured goes
      with them: an upload is idempotent now, so a second seeding replays it
- [x] Route tests: a mint, an identical replay, each conflicting field, an oversized body still
...
- [x] `client.uploadAsset` mints the id and `PUT`s. The capture flow is otherwise untouched: the
...
- [x] A reading for `asset-id-conflict` in `errors.ts`
- [x] `CONTEXT.md`: amend **Asset** — the id is minted by whoever uploads, so an asset has an
      identity before the pool holds its bytes, as a capture does. (**Pool identity** landed with
      the health route)
- [x] `docs/specs/http-v1.md`: the upload section and the refusal table. `docs/specs/core.md`: the
      asset id is the caller's. (`/v1/health`, core.md's pool identity read and
      `docs/specs/mirror.md` landed with the health route)
- [x] `pnpm test:stack` green, with a case that uploads under a minted id, replays it and conflicts
      on it. (The `/v1/health` case landed with the health route)
- [x] Add the dated `Shipped:` entries for the asset half (see Notes). (http-v1.md, mirror.md and
