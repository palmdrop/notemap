...
- [x] Create branch `agent/quieter-offline-marks`
- [x] The cached notice goes from the **queue and the feed** both. `CACHED` goes from
...
- [x] `surface()` loses `cached` and, with it, the `asked` flag and the `read()` wrapper that exists
...
- [x] The feed's `first` and the queue's `drained`/`bare` conditions still read `fromCache` and
      still need to: an empty surface nobody has answered for is not an empty pool
- [x] Tests: the queue and the feed draw no cache sentence while unreachable, and the refused
...
- [x] While the pool is out of reach, the foot of a surface with more to read says so and offers no
...
- [x] The word is drawn from the client's reachability, not from `fromCache`: a surface drawn from
...
- [x] The sentence lives in `$lib/said` with the others. It uses `offline`, the chrome's word
...
- [x] The queue and the feed drew the same foot already, so it became one `More` — the offline
      sentence has one home and the two surfaces differ only in which read they ask for
- [x] The foot keeps its `more` guard. A surface read to the end says nothing about being offline:
      there is nothing more to fetch either way
- [x] Tests: unreachable with more to read draws the sentence and no `load more` button; reachable
...
- [x] `packages/client/src/pool/reachability.ts` probes in **both** states: every ten seconds while
      ...
      this is about
- [x] Probing while reachable is the reversal of a stated decision — client.md says the probe "runs
...
- [x] The probe is **paused while nobody is looking**, and probes once on being looked at again, so
      the cost is one request per ten seconds per visible shell rather than per open tab forever
- [x] The visibility signal is the shell's — `document.visibilityState` and `visibilitychange` in
...
- [x] Tests, in `pool.test.ts`: a reachable pool is asked again after ten seconds; the backoff caps
...
- [x] [shell.md](../specs/shell.md)'s **A surface says what it is drawn from** is rewritten: the
      ...
      `Load more`
- [x] [client.md](../specs/client.md)'s **Reachability** section: the probe runs in both states, at
...
- [x] `CONTEXT.md` gains **Watched**
- [x] `Shipped:` entries on both specs, dated, linking here
- [x] This plan's Status set to `Done`
- [x] `git commit`
...
- **Whether the client-wide fake-timer tests start counting probes they did not before.** Settled
  2026-08-26: barely. Only `stops asking once the pool answers` was pinning the old behaviour, and it
  became its opposite. The shell's `pool()` helper now closes the client it replaces, which is what
  keeps a probe from outliving the test that started it.
- **Whether an integration or stack test leans on the probe stopping.** Settled 2026-08-26: none
  does; `pnpm test:stack` is green unchanged.
