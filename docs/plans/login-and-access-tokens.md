...
- [x] Create branch — `feature/auth` rather than the name below
- [x] An ADR: **the daemon authenticates; core does not, and the implementation is ours.** What it
...
- [x] Verify: the ADR is written and confirmed
- [x] `git commit`
...
- [x] **The seam first.** A small port inside the daemon answering two questions — *who is this
...
- [x] **The credential lives in the auth database, not in the config.** One row: a username, a hash,
...
- [x] **No credential set means no authentication**, exactly as today, and the daemon says so on
      ...
      phases 3 to 6 land
- [x] Password hashing: **`node:crypto` scrypt**. OWASP's first choice is Argon2id and scrypt is its
...
- [x] Tokens live in a small SQLite file the daemon owns, a sibling of the pool. **Not in the pool**:
      it is not the pool's material, it is not mirrored, and a rebuild is a new deployment that
      should issue new tokens
- [x] A session id and a token are both 32 random bytes from `randomBytes`, and **both are stored
...
- [x] A token carries a short identifying **prefix** so it is recognisable in a log or a config file
...
- [x] A token has a name, a creation time, an optional expiry and a last-used time. Expiry is checked
      on every use, and revocation takes effect on the next request — there is no cache to outrun
- [x] **A command line for the credential and the tokens**, as a subcommand on the existing bundle
...
- [x] It is **not user management**, and is not named as though it were: there is no user to manage,
      only a credential and some tokens. A command called `user` would smuggle back the concept this
      plan declined
- [x] There is no email in this, now or later: no verification, no magic link, no reset by mail. An
...
- [x] The running doc says how to reset a forgotten password, and that a daemon which will not start
      is recovered by running the image with the command overridden rather than by reaching into a
      container that is not up
- [x] Tests: a hash round-trips and a wrong password does not; an expired token does not
...
- [x] One middleware over `/v1` accepting either a session cookie or `Authorization: Bearer`. A
...
- [x] Open without a credential, and only these: `POST /v1/auth/login`, and `GET /v1/health` — the
      ...
      **liveness only** when unauthenticated: which pool this is, is a fact about the pool and goes
      behind the door
- [x] `POST /v1/auth/login`, `POST /v1/auth/logout`, `GET /v1/auth/session`. The cookie is
...
- [x] **A new session id is minted on every login**, and the old one invalidated. Session fixation is
      the thing this prevents and it is one line to omit
- [x] `GET`, `POST` and `DELETE` on the tokens routes. The secret appears in the creation response
...
- [x] **Authorization is deferred, and said so out loud** rather than left to be inferred from the
...
- [x] The shell's own files stay reachable without a credential: they are the application, not the
      pool, and something has to be able to draw the login. Everything they then *ask for* is behind
      the door
- [x] `/log` reads the action log, so the page goes behind it. `/docs` renders a document and calls
...
- [x] Tests: an unauthenticated `/log` does not answer pool material
- [x] Verify: `pnpm --filter @notemap/daemon test`
- [x] `git commit`
...
- [x] **A `401` is not a refusal.** Today a refusal is terminal: the entry becomes `refused`, is kept
...
- [x] The client carries a session state a surface can read, `login` and `logout`, and a transport
      that can carry a bearer token for a caller that is not a browser
- [x] **Signing out drops the cache and keeps the outbox** — the rule
...
- [x] Tests: a `401` mid-drain parks the outbox rather than refusing it; signing out clears the
...
- [x] A login surface in the register's own language — one field, the shell's bar above it, a
      refusal in the place refusals go. Not a modal: there is nothing behind it to return to
- [x] **An unauthenticated shell draws no pool material, cached or not.** The cache holds the pool's
...
- [x] What is still open, said plainly: no rate limiting beyond the login, no quotas, TLS is the
...
Recorded because the plan reads as though it did not.
...
- **scrypt is `N=2^16, r=8, p=2`**, the set OWASP lists beside its `N=2^17, r=8, p=1` floor as the
  same work for half the memory. 128 MiB a call is 512 MiB across libuv's threadpool, which is a
  lot to ask of the machines this runs on.
- **An access token may not end every session either.** The plan named one `403`; there are two,
  for the same containment reason.
- **Setting the password does not require the old one**, and neither does anything else the command
  line does. Whoever can run it can already read the database off the volume.
